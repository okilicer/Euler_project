...
print("We are going to solve Poisson Equation with homogeneous Dirichlet boundary condition with using central difference","\n")
...
x=sym.Symbol('x')    
expr1=(x-a)*(b-x)*exp(x)
real_solution_function=sym.lambdify(x,expr1,"numpy")
...
print("Note: We can notice that, the errors are increasing in the middle.  The estimation is getting worser, since we have information only at the boundary.") 
...
#otherwise it will overflow, use scaling to obtain [0,1]. 
